...
\begin{document}
\title{\LARGE Working with Two-Mode Networks in \pkg{multiplex}}
...
With \pkg{multiplex}, it is possible to perform an algebraic analysis of two-mode networks with the function \texttt{galois()} to produce Galois derivations between objects and attributes.
...
\subsubsection*{Reduced labeling of concepts}
...
Below is another Galois representation of the fruits dataset using using an \proglang{R} native pipe. 
...
For this, \pkg{multiplex} has a reverse dependence on \pkg{multigraph} for the visualization of multiple networks, and also for bipartite graphs. 
...
\subsection*{Bipartite graph with a binomial projection}

Another possibility to depict a two-mode network as a ``classic'' bipartite graph is to apply a force-directed layout to the binomial projection of this two-mode data set. 
The plot below is made with arguments in function \texttt{bmgraph()} for changing the aspect of the vertices and to make the graph clock-wise rotated. 
...
\bibitem{rgpahviz} Hansen KD, Gentry J, Long L, Gentleman R, Falcon S, Hahne F and Sarkar D  ~\pkg{Rgraphviz}: \emph{Provides plotting capabilities for R graph objects}. R package v 2.24.0
\bibitem{multiplex} Ostoic, JAR ~\pkg{multiplex}: \emph{Algebraic Tools for the Analysis of Multiple Social Networks}. R package v 3.7
\bibitem{multiplex} Ostoic, JAR ~\pkg{multigraph}: \emph{Plot and Manipulate Multigraphs}. R package v 0.99-4
%
\end{thebibliography}
