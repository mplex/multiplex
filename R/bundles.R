    ...
    sep) 
{
        ...
        if (isTRUE(TRD == TRUE) == TRUE) {
            ...
            for (k in 1:r) {
                ...
                assign(allr, transf(xd[, , k], type = "tolist", 
                  labels = lbs, sep = sep, lb2lb = lb2lb))
    ...
    As <- vector()
    for (i in 1:length(bd$asym)) {
        for (j in 1:length(bd$asym[[i]])) {
    ...
    Rp <- vector()
    for (i in 1:length(bd$recp)) {
        for (j in 1:length(bd$recp[[i]])) {
    ...
    Xc <- vector()
    if (isTRUE(TRD == TRUE) == TRUE) {
        for (i in 1:length(bd$xchg)) {
            for (j in 1:length(bd$xchg[[i]])) {
    ...
    Mx <- vector()
    if (isTRUE(TRD == TRUE) == TRUE) {
        for (i in 1:length(bd$mixe)) {
            for (j in 1:length(bd$mixe[[i]])) {
    ...
    Fl <- vector()
    if (isTRUE(TRD == TRUE) == TRUE) {
        for (i in 1:length(bd$full)) {
            for (j in 1:length(bd$full[[i]])) {
